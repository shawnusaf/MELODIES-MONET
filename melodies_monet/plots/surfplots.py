...
def make_diurnal_cycle(df, column=None, label=None, ax=None, avg_window=None, ylabel=None,
    ...
    ax = df[column].groupby(time.dt.hour).mean().plot(ax=ax, legend=True, **plot_kwargs) 
    max_vals = df[column].groupby(time.dt.hour).max()
    min_vals = df[column].groupby(time.dt.hour).min()
    ax.fill_between(max_vals, min_vals, color='grey', alpha=0.2)
